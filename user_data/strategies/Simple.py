...
class Simple(IStrategy):
    ...
    def populate_buy_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.buy_bb_enabled.value:
            conditions.append(
                dataframe[f"bb{self.bbwindow.value}upper{self.bbstd.value}"]
                > dataframe[f"bb{self.bbwindow.value}upper{self.bbstd.value}"].shift(1)
            )
    ...
    def populate_sell_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if self.sell_bb_enabled.value:
            conditions.append(
                dataframe[f"bb{self.bbwindow.value}lower{self.bbstd.value}"]
                < dataframe[f"bb{self.bbwindow.value}lower{self.bbstd.value}"].shift(1)
            )
